...
const (
	MaxSupportedRevision = 952
	MinSupportedRevision = 926
	GsNoErrors           = 0
	GsFatalError         = C.e_Fatal
)
...
func NewInstance() (*Ghostscript, error) {
	...
	if rev.Revision < MinSupportedRevision || rev.Revision > MaxSupportedRevision {
		return nil, fmt.Errorf("Ghostscript interpreter version not supported: %d (must be >%d, and <%d)", rev.Revision, MinSupportedRevision, MaxSupportedRevision)
	}

	var instance unsafe.Pointer
	if err := C.gsapi_new_instance(&instance, nil); err < GsNoErrors {
...
func (gs *Ghostscript) Exit() error {
	if err := C.gsapi_exit(gs.instance); err < GsNoErrors {
